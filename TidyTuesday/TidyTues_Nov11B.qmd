...
plot1 <- ggplot(who_tb_data_subset, aes(x = country, y = e_inc_100k)) +
    geom_point() +
    labs(title = "TB Cases per 100K by Country", y = "TB Cases per 100k") +  # Updated title and y-axis label
    theme(axis.title.x = element_blank())  # Remove x-axis title

# Save plot 1
ggsave("tb1.png", plot = plot1)

# Plot 2: Faceted scatter plot by region with jitter
plot2 <- ggplot(who_tb_data_subset, aes(x = country, y = e_inc_100k)) +
    geom_jitter(width = 0.2, height = 0) +  # Add jitter
    labs(title = "TB Cases per 100K by Region", y = "TB Cases per 100k") +  # Updated title and y-axis label
